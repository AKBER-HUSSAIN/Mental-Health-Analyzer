...
const Register = () => {
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-pink-100 via-blue-100 to-violet-100 dark:from-[#1a093b] dark:via-[#2d1a4d] dark:to-[#0a0a23] px-4">
            ...
            >
                <h2 className="text-3xl font-bold text-center bg-gradient-to-r from-pink-400 via-purple-400 to-blue-400 bg-clip-text text-transparent mb-2">Sign Up</h2>
                <input name="username" type="text" required placeholder="Username" className="rounded-lg p-3 bg-white/80 dark:bg-black/30 border border-white/30 focus:ring-2 focus:ring-purple-300 outline-none" value={form.username} onChange={handleChange} />
                ...
                <div className="text-center text-sm mt-2 text-gray-500 dark:text-gray-300">
                    Already have an account? <span className="underline cursor-pointer text-purple-500" onClick={() => navigate('/login')}>Login</span>
                </div>
            </motion.form>
        </div>
    );
