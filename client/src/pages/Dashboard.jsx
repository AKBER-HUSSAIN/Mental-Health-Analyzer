...
const Dashboard = () => {
    ...
    const analyzeEmotion = async () => {
        ...
        setResult(null);
        try {
            const response = await axios.post("http://localhost:5000/analyze", { text, email });
            ...
            <div className="relative z-20 flex flex-col min-h-screen w-full items-center justify-start">
                ...
                >
                    <motion.div className="flex items-center gap-3" whileHover={{ scale: 1.05 }}>
                        ...
                        <span className={`font-extrabold text-2xl bg-gradient-to-r from-pink-400 via-purple-400 to-blue-400 bg-clip-text text-transparent tracking-tight drop-shadow-md transition-colors duration-700`}>Dashboard</span>
                    </motion.div>
                ...
                >
                    ...
                    {/* View History Button */}
                    <motion.button onClick={() => navigate('/history')} className="mt-8 px-6 py-2 rounded-lg font-semibold bg-gradient-to-r from-blue-300 via-purple-300 to-pink-300 text-white shadow hover:scale-105 transition-all duration-200 dark:from-blue-600 dark:via-purple-600 dark:to-pink-600" whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.97 }}>
                        View History
                    </motion.button>
                </motion.div>
            </div>
